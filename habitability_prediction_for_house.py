# -*- coding: utf-8 -*-
"""Habitability_prediction_for_house.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1AS8qiHXWPJJY2Kn7-xSZ997cs5E7rS16
"""
...
d1['NR3']  = (d1['Neighborhood_Review']) * (d1['Neighborhood_Review'])
plot_data(d1,'NR2')

#d1['Habitability_score'], o = boxcox(d1['Habitability_score'])

#sns.kdeplot(d1['Habitability_score'])
...
d1.drop(columns =['Air_Quality_Index','Neighborhood_Review','Property_Area', 'NR2','NR1','Property_Area5',], axis = 1, inplace = True)
d1.head()
...
mean_target= d1.groupby('Property_Type')['Habitability_score'].mean()
d1['Property_Type'] = d1['Property_Type'].map(mean_target)
d1.head()
...
d1['Water_Supply'] = d1['Water_Supply'].replace("Once in a day - Morning","Once in a day")
d1['Water_Supply'] = d1['Water_Supply'].replace("Once in a day - Evening","Once in a day")
d1['Water_Supply'].value_counts()
...
d1['Water_Supply'] = d1['Water_Supply'].map(dict)
d1.head()
...
d1['Furnishing'] = d1['Furnishing'].map(dict1)
d1.head()
...
d1['Power_Backup'] = d1['Power_Backup'].map(dict2)
d1.head()
...
d1['Crime_Rate'] = d1['Crime_Rate'].map(dict3)
d1.head()
...
d1['Dust_and_Noise'] = d1['Dust_and_Noise'].map(dict4)
d1.head()

d1.drop(columns=['Property_ID'], axis =1, inplace= True)
d1.head()
...
df = final_df[['Furnishing','Crime_Rate','Power_Backup','NR3','Property_Area2','Traffic_Density_Score','Habitability_score']]
...
X_new =np.array([[4,4,3,24.8004,7.5224,3.98]])
X_new_df = pd.DataFrame(X_new, columns=['Furnishing', 'Crime_Rate', 'Power_Backup','NR3','Property_Area2','Traffic_Density_Score'])
X_new_df

sdata = scaler.transform(X_new_df)
sdata

result = model.predict(sdata)
result
...
pickle.dump(model, open('/content/drive/MyDrive/model.pickle', 'wb'))
